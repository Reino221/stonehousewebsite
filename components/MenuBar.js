...
const MenuBar = () => {
  ...
  const scaleIfSelected = (btn) => selected === btn
    ? { transform: 'scale(1.12)', zIndex: 1, transition: 'transform 0.18s cubic-bezier(.4,2,.6,1)' }
    : { transition: 'transform 0.18s cubic-bezier(.4,2,.6,1)' };

  const getPageTitle = () => {
    switch (router.pathname) {
      case '/refineries': return 'Refineries';
      case '/diesel-ulp': return 'FUEL DISTRIBUTION RSA';
      case '/stonehouse-estates': return 'Stonehouse Estates';
      case '/agriculture': return 'Agriculture';
      case '/minerals': return 'Minerals';
      case '/about': return 'About';
      case '/contact': return 'Contact';
      case '/transport': return 'Transport';
      case '/mineral-resources': return 'Mineral Resources';
      case '/agricultural': return 'Agricultural';
      default: return 'Stonehouse Holdings';
    }
  };
    ...
    <>
      ...
      >
        ...
        >
          ...
          />
          {isMobile ? (
            <span style={{ background: 'linear-gradient(90deg, #C99700, #FFD700)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontWeight: 'bold' }}>
              Stonehouse
            </span>
          ) : (
        ...
        {isMobile && (
          <button
            ...
            style={{ background: 'none', border: 'none', color: '#fff', fontSize: '1.5rem', cursor: 'pointer', padding: '0.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', transition: 'all 0.3s ease' }}
            aria-label="Toggle menu"
          >
            <div style={{ width: '25px', height: '3px', background: isMobileMenuOpen ? 'transparent' : '#fff', transition: 'all 0.3s ease', position: 'relative' }}>
              <div style={{ position: 'absolute', width: '25px', height: '3px', background: '#fff', top: isMobileMenuOpen ? '0' : '-8px', transform: isMobileMenuOpen ? 'rotate(45deg)' : 'rotate(0)', transition: 'all 0.3s ease' }} />
              <div style={{ position: 'absolute', width: '25px', height: '3px', background: '#fff', top: isMobileMenuOpen ? '0' : '8px', transform: isMobileMenuOpen ? 'rotate(-45deg)' : 'rotate(0)', transition: 'all 0.3s ease' }} />
            </div>
          </button>
        )}
        ...
        {isMobile && (
          <div style={{
            ...
            background: '#1D2A35', boxShadow: '0 8px 32px rgba(0,0,0,0.2)',
            borderTop: '1px solid #223044',
            transform: isMobileMenuOpen ? 'translateY(0)' : 'translateY(-100%)',
            opacity: isMobileMenuOpen ? 1 : 0,
            visibility: isMobileMenuOpen ? 'visible' : 'hidden',
            transition: 'all 0.3s ease', zIndex: 1199,
          }}>
